...
def _load_mc_field_definitions(type_record: str) -> pd.DataFrame:
    db = Database()
    fd = db.read_records(
...
def extract_1644_fields(
    ...
    field_defs = _load_mc_field_definitions("sms")
    ...
    dfs_by_fc: dict[str, list[pd.DataFrame]] = defaultdict(list)
